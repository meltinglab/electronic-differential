 ...
 * Component: Accel
 * Component Simulink Path: Accel
 * Simulation Mode: PIL
 *
 */

#define S_FUNCTION_NAME                Accel_pbs
    ...
    if ((pIsXILApplicationStarted!=NULL) && (*pIsXILApplicationStarted==1)) {
...

      {
        ...
        if (IOBufferPtr != NULL) {
          void * pXILService = (void *) ssGetPWorkValue(S, 9);
          if (pXILService != NULL) {
            if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 717, 0)!=
                XILHOSTAPPSVC_SUCCESS) {
              return;
            }                          /* if */
              ...
              *mxMemUnitPtr = (uint8_T) XIL_PROCESS_PARAMS_COMMAND;
...

              {
...

                {
                  uint8_T * simDataMemUnitPtr;
                  uint32_T offsetIdx;
                  for (offsetIdx=0; offsetIdx< 25; offsetIdx++) {
...

                    {
                      ...
                      simDataMemUnitPtr = (uint8_T*) (((char *)
                        ...
                        (S, "struct_qm0PExI0AxHWASzOnN0CqB"), offsetIdx));
...

              {
                ...
                void * dataInterfacePtr;
                dataInterfacePtr = (void *) ssGetRunTimeParamInfo(S, 7)->data;
      ...
      {
        {
          ...
          if (commandDispatch(S, IOBufferPtr, 717)!=XILHOSTAPPSVC_SUCCESS) {
            return;
          }                            /* if */
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("car_system_PIL");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
{
  {
    ...
    dims[0] = 1;
    dims[1] = 1;
    ssCheckSFcnParamValueAttribs(S, 0, "VEH", DYNAMICALLY_TYPED, 2, dims, 0);
  }

  {
    ...
    dims[0] = 4;
    dims[1] = 2;
    ssCheckSFcnParamValueAttribs(S, 1, "Klqr1", DYNAMICALLY_TYPED, 2, dims, 0);
  }

  {
    ...
    dims[0] = 4;
    dims[1] = 2;
    ssCheckSFcnParamValueAttribs(S, 2, "Klqr2", DYNAMICALLY_TYPED, 2, dims, 0);
  }

  {
    ...
    dims[0] = 4;
    dims[1] = 2;
    ssCheckSFcnParamValueAttribs(S, 3, "Klqr3", DYNAMICALLY_TYPED, 2, dims, 0);
  }

  {
    ...
    dims[0] = 4;
    dims[1] = 2;
    ssCheckSFcnParamValueAttribs(S, 4, "Klqr4", DYNAMICALLY_TYPED, 2, dims, 0);
  }

  {
    ...
    dims[0] = 4;
    dims[1] = 2;
    ssCheckSFcnParamValueAttribs(S, 5, "Klqr5", DYNAMICALLY_TYPED, 2, dims, 0);
  }

  {
    ...
    dims[0] = 1;
    dims[1] = 12;
    ssCheckSFcnParamValueAttribs(S, 6, "MotSpd", DYNAMICALLY_TYPED, 2, dims, 0);
  }

  {
    ...
    dims[0] = 1;
    dims[1] = 12;
    ssCheckSFcnParamValueAttribs(S, 7, "MotTrq", DYNAMICALLY_TYPED, 2, dims, 0);
  }
}
...
static void mdlSetWorkWidths(SimStruct *S)
{
  ...
  {
    uint32_T dtChecksum[4] = { 431203896U, 840154677U, 3615848503U, 3119877111U
    };
    ...
    ssRegDlgParamAsRunTimeParam(S, 0, 0, "P0_VEH", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }

  {
    ...
    /* set datatype */
    dataTypeId = 0;
    ssRegDlgParamAsRunTimeParam(S, 1, 1, "P1_Klqr1", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }

  {
    ...
    /* set datatype */
    dataTypeId = 0;
    ssRegDlgParamAsRunTimeParam(S, 2, 2, "P2_Klqr2", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }

  {
    ...
    /* set datatype */
    dataTypeId = 0;
    ssRegDlgParamAsRunTimeParam(S, 3, 3, "P3_Klqr3", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }

  {
    ...
    /* set datatype */
    dataTypeId = 0;
    ssRegDlgParamAsRunTimeParam(S, 4, 4, "P4_Klqr4", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }

  {
    ...
    /* set datatype */
    dataTypeId = 0;
    ssRegDlgParamAsRunTimeParam(S, 5, 5, "P5_Klqr5", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }

  {
    ...
    /* set datatype */
    dataTypeId = 0;
    ssRegDlgParamAsRunTimeParam(S, 6, 6, "P6_MotSpd", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }

  {
    ...
    /* set datatype */
    dataTypeId = 0;
    ssRegDlgParamAsRunTimeParam(S, 7, 7, "P7_MotTrq", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }
}
